	...
	if (employee_number && date && shift_id) {
		UsersModel.findOne({
			...
			ShiftType.findOne({
				...
				shift_type = data;
				// console.log((!employee.work_schedule || (employee.work_schedule && employee.work_schedule.break_type != 2))));
				if ((employee && (!employee.work_schedule || (employee.work_schedule && employee.work_schedule.break_type != 5))) && shift_type) { // Set as temporary fix (5) since weekender automatic schedule is not supported yet
					let user_id = employee.id;
					let check_in = shift_type && shift_type.time_in ? moment(shift_type.time_in).utc().format('HH:mm:ss') : null;
					let check_out = shift_type && shift_type.time_out ? moment(shift_type.time_out).utc().format('HH:mm:ss') : null;
					let shift_length = shift_type && shift_type.num_hours ? shift_type.num_hours : null;
					let type = shift_type && shift_type.shift_id ? shift_type.shift_id : null;
					let break_hour = shift_type && shift_type.break_hours ? shift_type.break_hours : 0;
								...
								if (!shift) {
									...
									ShiftsModel.create(reqData).then(data => {
										...
										TSSData.findOne({
											...
											tss = data;
											if (!tss) {
												TSSData.create(tssReqData).then(data => {
													resolve('resolved');
												});
											} else {
												resolve('resolved');
