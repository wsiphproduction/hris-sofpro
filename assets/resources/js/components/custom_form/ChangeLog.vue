<script>
    module.exports = {
        ...
        methods: {
            ...
            export_link(){
                let URL = base_url +'custom/change-log/export';
            ...
            setOldDate(event){
                ...
                if(this.form.type != 'update'){
                    ...
                    if(result){
                        this.form.old_login = result.actual_check_in ? moment(result.actual_check_in).utc().format('HH:mm') : '';
                        this.form.old_logout =result.actual_check_out ? moment(result.actual_check_out).utc().format('HH:mm') : '';
        	...
        	onChangeEmployee(event){
                ...
                let url = base_url + 'custom/change-log/getlog';
            ...
            editShift(index){
                ...
                let record = this.records;
                    record = record[index];
                let new_checkin = new Date(record.log_date);
                new_checkin = new_checkin.toISOString();
                this.form = {
                    id: record.id,
                    type: 'update',
                    employee: record.user_id,
                    approver_id: this.primary ? this.primary.primary_approver : 0,
                    approver_name: this.primary ? this.primary.first_name+' '+this.primary.last_name:'',
                    old_date: moment(record.log_date).format('ll'),
                    old_login: record.orig_login_time ? moment(record.orig_login_time).utc().format('HH:mm') : '',
                    old_logout: record.orig_logout_time ? moment(record.orig_logout_time).utc().format('HH:mm') : '',
                    new_date: new_checkin ? new_checkin : '',
                    new_login: record.req_login_time ? moment(record.req_login_time).utc().format('HH:mm') : '',
                    new_logout: record.req_logout_time ? moment(record.req_logout_time).utc().format('HH:mm') : '',
                    note: record.note,
                }
            ...
            initForm(){
                ...
                let url = base_url + 'custom/change-log/init';
            ...
            fetchRecord(){
                ...
                let REST = base_url + 'custom/change-log/fetch';
